...
const loadNode = async (node: Node, resolve: (data: Tree[]) => void) => {
  ...
  console.log(node)
  if (node.level === 0) {
    return resolve([{ name: publicStore.createSceneObj.type + '_tools', type: 'folder', id: 'tool_root_folder', description: "" }])
  }
  if (node.level === 1) {
...
const getToolList = (toolTye: string) => {
  return new Promise((resolve, reject) => {
    switch (toolTye) {
      case 'analysis':
        toolApi.getToolList(toolTye).then(res => {
          if (res.code === 200) {
            resolve(res.data)
          }
        }).catch(err => {
...
const handleCheckChange = (node: any, checked: any) => {
  ...
  switch (publicStore.createSceneObj.type) {
    case 'analysis':
      checked.checkedNodes.find((item: any) => {
        if (item.type === publicStore.createSceneObj.type) {

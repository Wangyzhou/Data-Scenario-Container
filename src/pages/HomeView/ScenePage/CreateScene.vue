<template>
  <div class="container">
    ...
    <div class="backBtnCon" v-if="createSceneCurrentStep !== 0" title="返回上一步">
      <el-icon size="30" @click="backStep">
        <ArrowLeftBold />
      </el-icon>
    </div>
    <div class="nextStepCon" v-if="createSceneCurrentStep !== 0 && createSceneCurrentStep !== 3" title="下一步">
...
import { ref, reactive, watch, onMounted, toRefs, onBeforeUnmount } from 'vue'
...
onMounted(() => {
  publicStore.createSceneObj.userId = userStore.loginUser.id
})
onBeforeUnmount(() => {
  publicStore.createSceneObj = {} as CreateSceneObj
  publicStore.createSceneCurrentStep = 0
})

const backStep = () => {
  active.value -= 1
  publicStore.createSceneCurrentStep = active.value
  router.push(`${active.value + 1}`)
}
const nextStep = () => {
  active.value += 1
  publicStore.createSceneCurrentStep = active.value

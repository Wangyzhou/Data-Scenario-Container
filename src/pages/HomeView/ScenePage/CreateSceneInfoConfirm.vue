  ...
  <div class="all">
    <el-form ref="ruleFormRef" :rules="rules" label-position="right" label-width="150px" :model="createSceneInfo"
      ...
      <el-form-item label="场景作者：" required>
        <el-input size="default" v-model="createSceneInfo.user.username" read-only="true" disabled />
      </el-form-item>
      <el-divider border-style="hidden" />
...
const createSceneInfo = reactive({
  ...
  type: publicStore.createSceneObj.type,
...
const submitCreateScene = (formEl: FormInstance) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      sceneApi.createScene({name: createSceneInfo.name, type: createSceneInfo.type, userId: createSceneInfo.user.id, dataSet: createSceneInfo.layers, toolSet: createSceneInfo.toolLab}).then(res => {
        if (res.code === 200) {
          console.log(res)
